...
from hurry.filesize import (
    size,
)  # had to import this from hurry.filesize 0.9-Python library for human readable file sizes
from hurry.filesize import (
    alternative,
)  # this is from that same package but it allows the size print out to look nicer
...
def file_size(directory):
    ...
    for x, y in pairs:
        ...
        print(output)
        print(
            "File Size Translate: ", size(y, system=alternative)
        )  # 'size' function if from the hurry.filesize library
        count = count + 1
