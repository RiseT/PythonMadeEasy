...
print(
    "Object donny is associated with the + ", donny.__class__.__name__, " + Class"
)  # prints 'Boss'
print(
    "Object judy is associated with the + ", judy.__class__.__name__, " + Class"
)  # prints 'Employee'
print(
    "Object donny is associated with the + ", donny.__class__.__bases__, " + Class"
)  # prints '(<class '__main__.Employee'>,) '
print("A List of all of Judy's attributes: ", list(judy.__dict__.keys()))

count = 1
for key in donny.__dict__:
    print(
        "Donny attrbute name {}: '{}' - And Value: '{}'".format(
            count, key, getattr(donny, key)
        )
    )
    count += 1
