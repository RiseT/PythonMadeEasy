...
def create_file(name):
    with open(name, "w") as create:
        ...
        writer = csv.DictWriter(
            create, fieldnames=fieldnames, lineterminator="\n"
    ...
    print("*" * 50)
    return (
        "'" + str(name) + "'" + " has been succesfully created!   ON DATE: " + str(now)
    )
...
for k, v in ref_count.items():
    print(
        "Item: {0:12} Count: {1} - {2}%".format(
            k, v, round((v / len(ref_orig) * 100), 1)
        )
    )
print("-" * 60)
